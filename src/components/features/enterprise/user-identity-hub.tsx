...
import { listUserLinks, type UserLinkItem } from "@/lib/api/flow-links";
...
export function UserIdentityHub() {
  ...
  const { data = [], isLoading, isError, error } = useQuery({
    queryKey: ["user", "links", status ?? "all"],
    queryFn: () => listUserLinks(getAccessToken, status),
  });

  const filtered = useMemo(() => {
    let list = data as UserLinkItem[];
    if (search.trim()) {
  ...
  return (
    <div className="w-full space-y-6">
      <PageHeader
        title={t("links.title.user")}
        description={t("links.desc.user")}
      />
      ...
      {isLoading ? (
        ...
        <>
          <SearchFilter
            ...
            placeholder={t("links.search.user")}
            filters={FILTERS}
            activeFilter={filter}
          ...
          <div className="overflow-hidden rounded-lg border border-neutral-200 bg-white dark:border-zinc-700 dark:bg-zinc-900">
            <div className="grid grid-cols-[3fr_1fr_1fr_auto] gap-2 border-b border-neutral-200 bg-neutral-50 px-4 py-2 text-[10px] font-semibold uppercase tracking-wider text-secondary dark:border-zinc-700 dark:bg-zinc-800">
              <span>{t("links.col.enterprise")}</span>
            ...
            {filtered.length === 0 ? (
              <div className="px-4 py-12">
                <EmptyState
                  title={t("links.empty.user.title")}
                  description={t("links.empty.user.desc")}
                />
              </div>
            ) : (
              <div className="divide-y divide-neutral-100 dark:divide-zinc-800">
                {filtered.map((link) => (
                  <div
                    key={link.link_id}
                    className="grid grid-cols-[3fr_1fr_1fr_auto] items-center gap-2 px-4 py-2 text-xs transition-colors hover:bg-neutral-50 dark:hover:bg-zinc-800/50"
                  >
