...
export function EnterpriseRegisterForm() {
  ...
  const [infoData, setInfoData] = useState({ nom_entreprise: "", username: "", phone: "" });
  const [error, setError] = useState<unknown>(null);

  const infoForm = useForm<InfoValues>({
    resolver: zodResolver(infoSchema),
    defaultValues: { nom_entreprise: "", username: "", phone: "" },
  });
  ...
  async function onPinSubmit(values: PinValues) {
    ...
    try {
      const raw = await registerEnterprise({
        ...
        username: infoData.username.trim(),
      ...
      const apiKey =
        raw && typeof raw === "object" && "api_key" in raw
          ...
          : "";
      const u = infoData.username.trim().toLowerCase();
      if (apiKey) {
        window.alert(
          u
            ? `${t("auth.enterpriseRegister.alertUsernameLine")} @${u}\n\n${t("auth.enterpriseRegister.alertApiKeyLine")}\n\n${apiKey}`
            : `${t("auth.enterpriseRegister.alertApiKeyLine")}\n\n${apiKey}`,
        );
      }
  ...
  return (
    <div className="space-y-4">
      ...
      {step === 1 ? (
        <div key="step-info" className="auth-slide-enter">
          ...
          >
            ...
            <div>
              <Label htmlFor="reg-ent-username" className="text-[11px]">
                {t("auth.register.usernameLabel")}
              </Label>
              <Input
                id="reg-ent-username"
                placeholder="acme_sas"
                className="h-9 text-xs"
                autoComplete="username"
                {...infoForm.register("username")}
              />
              {infoForm.formState.errors.username ? (
                <p className="mt-0.5 text-[11px] text-error" role="alert">
                  {infoForm.formState.errors.username.message}
                </p>
              ) : null}
            </div>
            <div>
      ...
      <p className="text-center text-[11px] text-secondary/70">
        {t("auth.enterpriseRegister.footer")}
      </p>
    </div>
  );
}
