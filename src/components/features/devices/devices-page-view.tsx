...
export function DevicesPageView() {
  const { t } = useI18n();
  const { data: user, isError: userErr, error: userError, refetch: refetchUser } = useUserQuery();
  const {
    data: historyRaw,
  ...
  const activeDevices = useMemo(() => {
    return user?.devices?.filter((d: any) => d.is_active) || [];
  }, [user]);
  ...
  async function handleDeactivate(deviceId: string) {
    ...
    try {
      const { deactivateDevice } = await import("@/lib/api/users");
      const { getAccessToken } = await import("@/lib/auth/session");
      await deactivateDevice(deviceId, getAccessToken);
      await refetchUser();
    } catch (e) {
      console.error(e);
    } finally {
  ...
  return (
    <div className="w-full space-y-8">
      ...
      <div className="space-y-4">
        <h2 className="text-sm font-semibold text-foreground">Appareils connectés</h2>
        <div className="grid gap-3 sm:grid-cols-2">
          {activeDevices.map((device: any) => (
            <div key={device.device_id} className="flex flex-col gap-3 rounded-xl border border-neutral-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-center gap-3">
                  <div className="rounded-lg bg-primary/10 p-2 text-primary">
                    <IconDevice className="size-5" />
                  </div>
                  <div>
                    <p className="text-sm font-bold text-foreground">{device.device_name || "Appareil inconnu"}</p>
              ...
              <div className="flex items-center gap-2 text-[10px] text-secondary">
                ...
                Dernière activité : {device.last_seen_at ? new Date(device.last_seen_at).toLocaleString() : "Inconnue"}
              </div>
            </div>
          ))}
          {activeDevices.length === 0 && (
            <p className="text-xs text-secondary py-4 text-center col-span-2 italic">Aucun appareil actif trouvé.</p>
          )}
        </div>
      </div>

      <hr className="border-neutral-200 dark:border-zinc-800" />

      {/* SECTION : HISTORIQUE DES CONNEXIONS */}
      <div className="space-y-4">
        <div className="flex items-center justify-between gap-3">
          <h2 className="text-sm font-semibold text-foreground">Historique des connexions</h2>
