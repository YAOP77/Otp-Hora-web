...
import { setUserRecoveryEmail, updateUserProfile, type UpdateUserProfilePayload } from "@/lib/api/users";
...
export default function AccountPage() {
  ...
  const editMutation = useMutation({
    mutationFn: async () => {
      ...
      const curUser = (data?.username ?? "").trim().toLowerCase();
      if (editUsername.trim().toLowerCase() !== curUser) payload.username = editUsername.trim();
  ...
  const infoRows = [
    { icon: IconUser, label: t("account.fullName"), value: fullName },
    { icon: IconUser, label: t("account.username"), value: username },
    { icon: IconCheck, label: t("common.status"), value: status },
    { icon: IconPhone, label: `${t("common.phone")} (Figé)`, value: phone },
    { icon: IconMail, label: t("common.email"), value: email },
  ];
    ...
    <div className="flex w-full gap-6">
          ...
          <div className="flex flex-col gap-6">
            ...
            <section className="max-w-2xl rounded-lg border border-neutral-200 bg-white p-5 dark:border-zinc-700 dark:bg-zinc-900 sm:p-6">
              <div className="flex items-start justify-between">
                <h2 className="text-base font-bold text-foreground">{t("account.info")}</h2>
                {userId ? (
      ...
      <Modal open={editOpen} onClose={() => setEditOpen(false)} title="Modifier le profil" footer={<><Button variant="secondary" className="h-9 gap-1.5 text-xs" onClick={() => setEditOpen(false)}><IconX className="size-3.5" />Annuler</Button><Button className="h-9 gap-1.5 bg-[#0B3A6E] text-xs hover:bg-[#0B3A6E]/90" onClick={() => editMutation.mutate()} loading={editMutation.isPending}><IconSave className="size-3.5" />Enregistrer</Button></>}>
        <div className="space-y-3">
          ...
          <div><Label htmlFor="edit-username" className="text-[11px]">{t("account.username")}</Label><Input id="edit-username" value={editUsername} onChange={(e) => setEditUsername(e.target.value)} className="h-9 text-xs" autoComplete="username" placeholder="ex. jean_thomas" /></div>
